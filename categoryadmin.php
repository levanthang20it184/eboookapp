								...
								<?php
									...
									{ ?>
										...
										<td>
											...
											<?php 
												if ($demoOrLive == "1") { ?>
													<a href="?cat_id=<?php echo $row['cat_id'];?>" class="btn btn-danger" onclick="return confirmDelete('<?php echo $row['cat_id'];?>')"><i class="fa fa-trash-o"></i></a>
											<?php } else {?>
...
function confirmDelete(newsId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = '?cat_id=' + newsId;
        }
    });
